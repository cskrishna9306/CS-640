...
def getoptions(argv):
   ...
   opts, _ = getopt.getopt(argv,"p:g:r:q:l:f:e:i:t")
   opts = dict(opts)

   return int(opts['-p']), int(opts['-g']), int(opts['-r']), int(opts['-q']), int(opts['-l']), opts['-f'], int(opts['-e']), int(opts['-i']), int(opts['-t'])
...
if __name__ == "__main__":
   ...
   opts, _ = getopt.getopt(sys.arvs[1:],"p:g:r:q:l:f:e:i:t")
   opts = dict(opts)
   ...
   # TODO: Record loss rate, total transmissions